  ...
  return (
    <div style={{
      ...
      <button
        ...
        style={{
          fontSize: 12, fontWeight: 600, color: cooldown > 0 ? 'var(--stone)' : 'var(--signal)',
          background: 'none', border: 'none', cursor: cooldown > 0 ? 'default' : 'pointer',
          display: 'flex', alignItems: 'center', gap: 4,
          opacity: (isSending || cooldown > 0) ? 0.7 : 1, padding: 0,
        }}
      >
        {isSending && <ArrowPathIcon style={{ width: 12, height: 12 }} />}
        {isSending ? 'Enviando…' : cooldown > 0 ? `Reenviar en ${cooldown}s` : 'Reenviar correo'}
      </button>
      <button
