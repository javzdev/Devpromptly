...
// ─── Modal ────────────────────────────────────────────────────────────────────
const CommunityModal: React.FC<{ forum: any; onClose: () => void }> = ({ forum, onClose }) => {
  const platform = getPlatform(forum.url);
  const pStyle   = PLATFORM_STYLES[platform];
  const lang     = forum.language === 'en' ? LANG_STYLES.en : LANG_STYLES.es;
...
// ─── Card ─────────────────────────────────────────────────────────────────────
const CommunityCard: React.FC<{ forum: any; onClick: () => void }> = ({ forum, onClick }) => {
  const platform = getPlatform(forum.url);
  const pStyle   = PLATFORM_STYLES[platform];
  const lang     = forum.language === 'en' ? LANG_STYLES.en : LANG_STYLES.es;
  ...
  return (
    <div style={{ background: 'var(--void)', minHeight: '100vh' }}>
      ...
      <div className="border-b border-white/5 pb-12 pt-8 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <div style={{
            ...
            borderRadius: 99, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)',
            fontSize: 10, fontWeight: 700, fontFamily: 'JetBrains Mono, monospace',
            letterSpacing: '0.1em', textTransform: 'uppercase' as const, color: 'var(--signal)', marginBottom: 24,
          }}>
          ...
          <p style={{ fontSize: 15, color: 'var(--stone)', lineHeight: 1.7, maxWidth: 480, margin: '0 auto 32px' }}>
            {allForums.length > 0 && <span style={{ color: 'var(--parchment)', fontWeight: 700 }}>{allForums.length}+ comunidades </span>}
            de Reddit, Discord, foros y más — en español e inglés. Haz clic para ver detalles y unirte.
          </p>
          ...
          <div className="relative max-w-md mx-auto">
            ...
            <input
              ...
              style={{
                ...
                borderRadius: 'var(--r-sm)', fontSize: 14, color: 'var(--parchment)', outline: 'none', transition: '160ms',
              }}
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        {/* Platform filter */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, overflowX: 'auto', paddingBottom: 8, marginBottom: 10 }}>
          {/* All */}
          <button
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filtered.map(forum => (
                <CommunityCard key={forum._id} forum={forum} onClick={() => setSelected(forum)} />
              ))}
            </div>
          </>
        )}
      ...
      {/* Modal */}
      {selected && <CommunityModal forum={selected} onClose={handleClose} />}
    </div>
  );
};
